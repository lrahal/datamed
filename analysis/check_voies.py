...
def get_diff_voies():
    ...
    for c in cis_list:
        if (
            r[c].get(2018, "")
            and r[c].get(2019, "")
            and r[c].get(2018, "") != r[c].get(2019, "")
        ):
            bad_voie.append(
                {"cis": c, "2018": r[c].get(2018, ""), "2019": r[c].get(2019, "")}
            )
    ...
    df_2017["VOIE 4 classes"] = df_2017["code CIS"].apply(
        lambda x: voie_4_by_cis.get(x, None)
    )
    ...
    df_2017["matched"] = df_2017["code CIS"].apply(
        lambda x: "NOK" if x in cis_not_matched else "OK"
    )
